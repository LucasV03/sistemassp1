...
type Props = {
  label?: string;
  valueName: string; // nombre seleccionado (string)
...
export default function SelectCategoria({
  ...
  valueName,
  onPick,
  placeholder = "Buscar/crear categoría...",
  autoCreate = true,
}: Props) {
  const [q, setQ] = useState(valueName ?? "");
  const debounced = useDebounce(q, 250);

  // ⬇️ la query devuelve { items, nextCursor }; tomamos items
  const data = useQuery(api.categorias.buscar, { q: debounced, limit: 20, cursor: undefined });
  const resultados = data?.items ?? [];

  const crear = useMutation(api.categorias.crear);
