...
type Props = {
  ...
  placeholder?: string;
  autoCreate?: boolean;
  marcaId?: string | null; // opcional, para filtrar/crear
};
...
export default function SelectVehiculo({
  ...
  valueName,
  onPick,
  placeholder = "Buscar/crear vehículo...",
  autoCreate = true,
  marcaId = null,
}: Props) {
  ...
  const data = useQuery(api.vehiculos.buscar, {
    q: debounced,
    limit: 20,
    cursor: undefined,
    marcaId: marcaId ? (marcaId as any) : undefined, // la API espera Id<"marcas">
  });
          ...
          <div className="absolute z-20 mt-1 w-full max-h-64 overflow-auto rounded border border-neutral-700 bg-neutral-900 shadow">
            ...
            {canCreate && (
              <button
                type="button"
                onClick={async () => {
                  const id = await crear({ nombre: q.trim(), marcaId: marcaId as any });
            ...
            {!marcaId && (
              <div className="px-3 py-2 text-xs text-neutral-400">
                (Seleccioná una marca para crear un vehículo)
              </div>
            )}
          </div>
        )}
