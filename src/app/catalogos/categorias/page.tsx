...
export default function CategoriasPage() {
  ...
  const res = useQuery(api.categorias.buscar, { q, limit: 100, cursor: undefined });
  const items = res?.items ?? [];

  const crear = useMutation(api.categorias.crear);
  const eliminar = useMutation(api.categorias.eliminar);
  ...
  const handleCrear = async (e: React.FormEvent) => {
    ...
    setNuevoNombre("");
    // Convex rehidrata la lista automáticamente
  };
            ...
            {items.length === 0 && (
              <tr>
                <td className="p-4 text-neutral-400" colSpan={2}>
                  {q ? "Sin coincidencias." : "Aún no hay categorías."}
                </td>
              </tr>
            )}
          </tbody>
