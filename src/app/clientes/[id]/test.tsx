...
import { useParams, useRouter } from "next/navigation"; // ⬅️ agregar useRouter
...
export default function DetalleCliente() {
  ...
  // ====== Queries principales (siempre se llaman en el mismo orden) ======
  ...
  const interacciones =
    useQuery(api.interacciones.interaccionesPorCliente, { clienteId: id as any }) ?? [];
  ...
  const [archivos, setArchivos] = useState<File[]>([]); // selección múltiple

  // ====== Historial y adjuntos del contrato en edición ======
  const historial =
    useQuery(
  ...
  // ====== Estado Interacciones (crear/editar) — ¡ANTES de cualquier return! ======
...
async function subirYAgregarAdjuntos() {
  ...
  const permitidos = new Set([
    ...
    "image/tiff",
    "image/svg+xml",
    // Nota: "image/*" ya cubre varios, pero mantenemos lista explícita.
  ]);

  const elegibles = archivos.filter(f => {
    // algunos navegadores pueden dejar vacío file.type; en ese caso validamos por extensión simple
  ...
  const guardarContrato = async (e: React.FormEvent) => {
    ...
    try {
      if (cIdEdit) {
        ...
        setCOk("Contrato registrado con éxito.");
      }
      limpiarContrato();
    } catch (err: any) {
  ...
  // ====== (después de declarar TODOS los hooks) ======
          ...
          <div className="bg-white rounded-2xl shadow p-6 text-black space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <input
                ...
                className="hidden"
                multiple
                accept="image/*,.pdf,application/pdf"   // ⬅️ solo imágenes y PDF
                onChange={(e) => setArchivos(Array.from(e.target.files ?? []))}

              />
              <label
                htmlFor="multi-archivos"
                className="inline-flex items-center px-4 py-2 rounded bg-black text-white font-semibold hover:bg-gray-800 cursor-pointer"
              >
